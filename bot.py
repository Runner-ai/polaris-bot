...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        f"⏳ Cooldown: /sum — {COOLDOWN_MINUTES} мин, "
        f"/ask и /imagine — {COOLDOWN_ASK_SECONDS} сек",
        parse_mode="Markdown"
    )
...
async def weather_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("⏱ Сервис погоды не отвечает. Попробуй позже.")
    except Exception as e:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Ты умный и дружелюбный помощник в групповом чате друзей. "
                        "Отвечай по делу. "
